...
class ReloadHandler(FileSystemEventHandler):
    def __init__(self, restart_callback, *extensions:  str | tuple[str]):
    ...
    def on_modified(self, event):
        ...
        if event.src_path.endswith(self.extensions):
...
class AutoReloadServer:
    def __init__(self, host="0.0.0.0", port=8000, directory='.', *extensions: tuple[str]):
        self.port = port
        self.host = host
        self.directory = directory
        self.server = None
    ...
    def run_with_autoreload(self):
        ...
        event_handler = ReloadHandler(self.restart_server, *self.watched_extensions)
        observer = Observer()
